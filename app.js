...
// Coinremitter mock API endpoints
app.post('/api/coinremitter/create-invoice', async (req, res) => {
    ...
    // You would call Coinremitter API here in production
    res.json({
        success: true,
        invoice: {
            invoice_id: 'MOCKINV-' + Date.now(),
            total_amount: (amount / 50000).toFixed(6), // mock BTC conversion
            address: '1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa',
            qr_code: 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa',
...
app.get('/api/coinremitter/check-availability', (req, res) => {
    res.json({ success: true, availableCryptos: ['BTC', 'ETH', 'LTC', 'DOGE', 'USDT'] });
});

app.get('/api/coinremitter/payment-status/:invoiceId', (req, res) => {
    // Always return paid for demo
    res.json({ success: true, status: 'paid', crypto: 'BTC', transactionHash: 'MOCKTXN-' + req.params.invoiceId });
});
